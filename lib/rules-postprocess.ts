export function postProcessBrastaRules(previousState: any, command: any, result: any): any {
  ...
  const wasLastHand = previousState?.lastHandRound === previousState?.round;
  ...
  if (finalDealStarted) next.lastHandRound = next.round;
  else if (wasLastHand && next.round === previousState.round) next.lastHandRound = previousState.lastHandRound;

  if (next.phase === 'play' && next.lastHandRound === next.round) {
    if (!(next.event || '').includes('LAST HAND!')) {
      next.event = next.event ? `${next.event} • LAST HAND!` : 'LAST HAND!';
    }
    if (finalDealStarted) next.message = `LAST HAND! Final four-card deal. Seat ${next.currentSeat}'s turn.`;
  }
