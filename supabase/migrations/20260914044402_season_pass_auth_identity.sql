-- PostgREST supplies the JWT as request.jwt.claims. Use Supabase auth.uid()
-- so valid sessions resolve correctly while cross-account requests stay denied.
...
begin
  if v_player_id is null or p_player_id is distinct from v_player_id then
