(() => {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .ranked-match-found{width:min(620px,100%);text-align:center;color:#f7f2e8;padding:44px 32px;border:1px solid #d8b75e55;border-radius:28px;background:linear-gradient(180deg,#0c291eeb,#061711f2);box-shadow:0 30px 100px #0009,0 0 70px #d8b75e0b;position:relative;overflow:hidden}
      ...
      .ranked-matchup{display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:16px;margin:0 auto 24px;max-width:500px}
    ...
    lobby.innerHTML = `
      <section class="ranked-match-found" aria-live="polite">
        ...
        <div class="ranked-matchup">
          <div class="ranked-player"><small>You</small><strong>${esc(match.name || 'You')}</strong></div>
          <div class="ranked-vs">VS</div>
          <div class="ranked-player"><small>Opponent</small><strong>${esc(match.opponent || 'Opponent')}</strong></div>
        </div>
        <div class="ranked-connect-pulse" aria-hidden="true"><i></i><i></i><i></i></div>
        <p class="ranked-connect-copy">Connecting both players…</p>
      </section>`;
  }
