(() => {
  ...
  function findLearnCard() {
    ...
    return heading?.closest('.landing-card, .home-card, .product-card, section, article, div') || null;
  }
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .bot-difficulty-options button{min-height:44px;border-radius:14px;border:1px solid rgba(224,188,86,.34);background:rgba(8,40,27,.74);font-weight:800;color:#f5f0df}
  ...
  function ensureDifficultyPanel() {
    ...
    if (!panel) {
      ...
      if (playButton?.parentElement) playButton.parentElement.insertBefore(panel, playButton);
  ...
  function boot() {
    ...
    new MutationObserver(ensureDifficultyPanel).observe(root, { childList: true, subtree: true });
